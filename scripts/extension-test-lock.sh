...
case "$cmd" in
  ...
  install)
    ...
    restore_ref=$(cat "$LOCK/restore" 2>/dev/null || printf 'main')
    ...
    here=$(slot_head)
    installed=$(cat "$LOCK/installed" 2>/dev/null || :)
    if [ -n "$(slot_branch)" ]; then
      [ "$(slot_branch)" = "$restore_ref" ] ||
        die "the main checkout is on $(slot_branch), not $restore_ref -- put it back before installing"
    elif [ "$here" != "$installed" ]; then
      die "the main checkout is detached at $(short HEAD) and this lock did not put it there -- run \`$0 break\` first"
    fi
    if [ -n "$(pending)" ]; then
    ...
    reload=
    git -C "$ROOT" diff --quiet "$here" "$sha" -- "$MANIFEST" 2>/dev/null || reload=1

    git -C "$ROOT" checkout --quiet --detach "$sha"
    printf '%s\n' "$sha" > "$LOCK/installed"
    printf 'installed %s (%s) in %s\n' "$(git -C "$SELF" rev-parse --abbrev-ref HEAD)" "$(short "$sha")" "$ROOT"
    if [ -n "$reload" ]; then
      printf '%s changed -- press the reload button on the extension'"'"'s card at chrome://extensions before testing\n' "$MANIFEST"
    fi
    ;;
  ...
  release)
    ...
    if [ "$mode" != "--force" ] && [ "$(slot_head)" != "$installed" ]; then
      printf 'The main checkout is at %s, not the %s this lock installed.\n' "$(short HEAD)" "$(short "$installed")" >&2
